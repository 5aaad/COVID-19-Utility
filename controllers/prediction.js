...
exports.timeSeries = asyncHandler(async (req, res, next) => {
	...
	pythonScript().then(() => (
		...
		fs.readFile('C:/Users/Saad Ur Rehman/Desktop/realTimeChatApp-main/covid-model-files/time.txt', 'utf8', async (err, data) => {
			if (err) {
				console.error(err)
				return
			}
			...
			// res.send(timeSeriesResult)
			// console.log(JSON.parse(timeSeriesResult.yhat_lower))
			const dataArray = [];
			...
			// res.send(timeSeriesResult.yhat_lower)
		})
